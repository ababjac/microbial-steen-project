...
def plot_confusion_matrix(y_pred, y_actual, title, filename):
    ...
    plt.tight_layout()
    plt.savefig('images/SVM/confusion-matrix/malaria/Lasso/'+filename)
    plt.close()
...
def plot_auc(y_pred, y_actual, title, filename):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig('images/SVM/AUC/malaria/Lasso/'+filename)
    plt.close()
...
expr_features = pd.read_csv('files/data/new_expression.csv', header=0, index_col=0, encoding=detect_encoding('files/data/new_expression.csv')).fillna(0)
expr_features = expr_features.T
expr_features = expr_features.reset_index()
...
plot_auc(probs, y_test, 'AUC for '+label, label+'_AUC-newexpr-nometa.png')
...
plot_confusion_matrix(y_pred=y_pred, y_actual=y_test, title=label, filename=label+'_CM-newexpr-nometa.png')
