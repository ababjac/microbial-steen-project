...
features = data.loc[:, ~data.columns.isin(['genome_id', 'cultured.status', 'culture.level', 'taxonomic.dist', 'completeness'])]#, 'domain', 'phylum', 'class', 'order', 'family', 'genus', 'species', 'completeness'])] #get rid of labels
...
plot_auc(probs, y_test, 'AUC for '+label, label+'_AUC-no-obvious.png')
...
plot_confusion_matrix(y_pred=y_pred, y_actual=y_test, title=label, filename=label+'_CM-no-obvious.png')
